...
async function initDatabase() {
    ...
    try {
        const feesCount = await db.prepare('SELECT COUNT(*) as c FROM cuotas_fees').get();
        if (feesCount.c === 0) {
            const fees = [
            ...
            for (const f of fees) {
                await db.prepare('INSERT INTO cuotas_fees (plan, cuotas, fee_cobro_pct, fee_cuotas_pct, iibb_pct, posnet_pct) VALUES (?, ?, ?, ?, ?, ?)').run(f.plan, f.cuotas, f.fee_cobro_pct, f.fee_cuotas_pct, f.iibb_pct, f.posnet_pct);
            }
    ...
    if (existsSync(seedPath)) {
        try {
            const count = await db.prepare('SELECT COUNT(*) as c FROM catalogo').get();
            if (count.c === 0) {
        ...
        `);
                for (const item of seed) {
                    await stmt.run(item.producto, item.modelo, item.descripcion || '', item.precio_contado_usd, item.precio_regular_usd, item.cuotas_3, item.cuotas_6, item.cuotas_9, item.cuotas_12, item.categoria, item.destacado || 0);
                }
...
app.get('/api/status', asyncHandler(async (req, res) => {
    const catalogoCount = (await db.prepare('SELECT COUNT(*) as c FROM catalogo').get()).c;
    const usersCount = (await db.prepare('SELECT COUNT(*) as c FROM users').get()).c;
...
app.post('/api/auth/login', asyncHandler(async (req, res) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email, rol: user.rol, nombre: user.nombre }, JWT_SECRET);
...
app.get('/api/ventas/:id', authMiddleware, asyncHandler(async (req, res) => {
    ...
    if (req.user.rol !== 'admin' && item.closer_id !== req.user.id) {
        return res.status(403).json({ error: 'No autorizado' });
    }
    res.json(item);
}));
app.post('/api/ventas', authMiddleware, asyncHandler(async (req, res) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(closerId, data.nombre_comprador || '', data.apellido_comprador || '', data.dni || '', data.producto, precioVenta, costo, ganancia, comision, data.pago_completo ? 1 : 0, montoSenado, faltaPagar, data.metodo_pago || 'Efectivo USD', esCanje, data.estado || 'Completada', data.notas || '', data.comprobante_pdf || null);
  ...
  `).run(ventaId, closerId, facturaNum, `${data.nombre_comprador || ''} ${data.apellido_comprador || ''}`.trim(), data.dni || '', data.producto, precioVenta, montoSenado, faltaPagar, esCanje);
...
app.put('/api/ventas/:id', authMiddleware, asyncHandler(async (req, res) => {
    const data = req.body;
    const venta = await db.prepare('SELECT * FROM ventas WHERE id = ?').get(req.params.id);
    if (!venta)
        return res.status(404).json({ error: 'No encontrado' });
    ...
    if (data.nombre_comprador !== undefined || data.apellido_comprador !== undefined || data.dni !== undefined) {
        await db.prepare('UPDATE facturas SET cliente_nombre = ?, cliente_dni = ? WHERE venta_id = ?').run(`${data.nombre_comprador || venta.nombre_comprador || ''} ${data.apellido_comprador || venta.apellido_comprador || ''}`.trim(), data.dni || venta.dni || '', req.params.id);
    }
    res.json({ success: true, comision_usd: comision });
}));
app.delete('/api/ventas/:id', authMiddleware, asyncHandler(async (req, res) => {
    const venta = await db.prepare('SELECT closer_id FROM ventas WHERE id = ?').get(req.params.id);
    if (!venta)
        return res.status(404).json({ error: 'No encontrado' });
    if (req.user.rol !== 'admin' && venta.closer_id !== req.user.id) {
        return res.status(403).json({ error: 'No autorizado' });
...
app.post('/api/cuotero/calcular', authMiddleware, asyncHandler(async (req, res) => {
    ...
    if (pUsd <= 0 || tc <= 0)
        return res.status(400).json({ error: 'Precio USD y tipo de cambio son requeridos' });
    const fee = await db.prepare('SELECT * FROM cuotas_fees WHERE plan = ?').get(plan);
    if (!fee)
...
app.get('/api/metricas', authMiddleware, asyncHandler(async (req, res) => {
    const closerId = req.user.rol === 'admin' && req.query.closer_id ? parseInt(req.query.closer_id) : req.user.id;
...
app.post('/api/turnos', authMiddleware, asyncHandler(async (req, res) => {
  ...
  `).run(d.titulo || `${d.cliente_nombre || ''} - ${d.motivo || 'Consulta'}`, d.cliente_nombre || '', d.telefono || '', d.fecha_hora, d.motivo || 'Consulta', d.producto_objetivo || 'Otro', d.modelo_detalle || '', d.que_busca || '', parseFloat(d.presupuesto_estimado) || 0, d.moneda || 'USD', d.forma_pago || 'Efectivo', d.senia || 'No aplica', parseFloat(d.monto_senia) || 0, d.cliente_id ? parseInt(d.cliente_id) : null, d.venta_id ? parseInt(d.venta_id) : null, closerId, d.confirmado || 'Sin confirmar', d.canal_contacto || 'WhatsApp', d.ultimo_contacto || null, d.estado_recordatorio || 'Pendiente', d.tipo || 'Consulta', d.estado || 'Pendiente', d.notas || '', d.notificar_whatsapp ? 1 : 0);
    res.json({ id: result.lastInsertRowid });
}));
app.put('/api/turnos/:id', authMiddleware, asyncHandler(async (req, res) => {
    const turno = await db.prepare('SELECT closer_id FROM turnos WHERE id = ?').get(req.params.id);
    if (!turno)
        return res.status(404).json({ error: 'No encontrado' });
...
app.post('/api/stock/:id/ajustar', authMiddleware, adminOnly, asyncHandler(async (req, res) => {
    ...
    const item = await db.prepare('SELECT * FROM stock WHERE id = ?').get(req.params.id);
    if (!item)
        return res.status(404).json({ error: 'No encontrado' });
